/* CyberDefender IDS v1.0 — API Client, Auth, Charts, Real Model Metrics, Real-Time IDS
   FIXES:
...
const CG = {
  ...
  pdfReport: async data => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'cyberdefender_report.pdf'; a.click(); URL.revokeObjectURL(url);
    return {success:true};
  },
};
...
const TICKER_ITEMS = [
  ...
  {c:'g', m:'INFO · CyberDefender IDS v1.0 ONLINE · 6 real models · RF=100% NSL-KDD · XGB=100% · ANN=100% · AE=98.3% · Hybrid=84.3%'},
...
function initUserChip() {
  ...
  document.getElementById('userChip')?.addEventListener('click', () => {
    if (confirm('Sign out of CyberDefender IDS?')) { clearSession(); window.location.href = '../pages/login.html'; }
  });
}
