"""CyberDefender IDS — Backend Utilities
FIXES:
...
log = logging.getLogger("cyberdefender")
...
DEFAULTS = [
    {"username":"admin",   "password":"CyberDefender2024!", "role":"admin",   "email":"admin@cyberdefender.local"},
    {"username":"analyst", "password":"Analyst2024!",    "role":"analyst", "email":"analyst@cyberdefender.local"},
    {"username":"viewer",  "password":"Viewer2024!",     "role":"viewer",  "email":"viewer@cyberdefender.local"},
]
...
def build_pdf(data, requester={}):
    try:
        ...
        # ── Title banner ──────────────────────────────────────────────────
        banner = Table([[
            P("⚡  CYBERDEFENDER IDS v1.0", bold=True, color=C_HDR_TEXT, size=19),
        ...
        story.append(P(
            "CyberDefender IDS v1.0  ·  NSL-KDD + UNSW-NB15  ·  "
...
<div style="max-width:660px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,.15)">
  ...
  <div style="background:#0a1520;padding:24px 28px">
    <h1 style="color:#e0f0ee;margin:0;font-size:20px;letter-spacing:1px">⚡ CyberDefender IDS — Security Report</h1>
  ...
  <div style="padding:28px">
    ...
    <p style="color:#333;font-size:13px;margin-bottom:20px">Your CyberDefender IDS scan has completed. The threat assessment is below.</p>
    ...
    <p style="font-size:11px;color:#556677;margin:0">CyberDefender IDS v1.0 &nbsp;·&nbsp; {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')} &nbsp;·&nbsp; CONFIDENTIAL</p>
  </div>
</div>
</body></html>"""
...
def send_slack(message, severity="info"):
    ...
    payload = {"attachments":[{
        "color": colour,
        "text":  f"{emoji} *CyberDefender IDS* — {message}",
...
def to_cef(e):
    ...
    return (f"CEF:0|CyberDefender|IDSv1|1.0|{e.get('label','?')}|"
            f"{e.get('attack_family','?')}|{sn}|"
            f"src={e.get('src_ip','?')} dst={e.get('target','?')} "
            f"cs1={e.get('mitre_tactic','?')} rt={e.get('timestamp','?')}")

def to_leef(e):
    return (f"LEEF:2.0|CyberDefender|IDSv1|1.0|{e.get('label','?')}|"
